...
const knex = require('knex')({
  ...
  connection: {
    host : '127.0.0.1',
    user : 'm4st3rx',
    password : 'LeedsVaros2016',
    database : 'fortnite-itemset'
  }
});
...
app.get('/fn/set/:hash', function(request, response){
  ...
  .then(function (values) {
    ...
    values.forEach(function(resp, index, originalArr){
      if(resp.data.status != 200) throw new Error("Error occured"); //response.end("Error occured.");
      if(resp.data.data.length < 1) throw new Error("\"" + resp.data.query.search + "\" item not found"); //response.end();
